...
- Plan 23 (generalize OIDF entity consumption) is the follow-on to Plans 21 and 24 that turns the current demo-local OIDF resolver into a generic allowlist-based consumer. Phases 1 through 5 (config model rewrite, generic URL/fetch utilities, discovery-driven issuer trust, allowlist enforcement, and demo-topology compatibility) are implemented on `main`.
