...
import type { ClientBinding, RegisteredClient } from "../store/model.ts";
...
export class ClientRegistry {
  ...
  registerUdap(input: { frameworkBinding: ClientBinding; clientName?: string; scope?: string }) {
    ...
    this.activeUdapRegistrations.set(this.udapRegistrationKey(input.frameworkBinding), hashClientId(client.clientId));
    this.clients.set(client.clientId, client);
    return client;
  }

  cancelUdap(frameworkBinding: ClientBinding) {
    if (frameworkBinding.framework_type !== "udap") {
      throw new Error("UDAP cancellation requires a UDAP framework binding");
    }
    this.activeUdapRegistrations.set(this.udapRegistrationKey(frameworkBinding), null);
  }
  ...
  private isActiveUdapClient(client: RegisteredClient) {
    ...
    const activeHash = this.activeUdapRegistrations.get(this.udapRegistrationKey(client.frameworkBinding));
    if (activeHash === undefined) return true;
    if (activeHash === null) return false;
    return activeHash === hashClientId(client.clientId);
  }

  private udapRegistrationKey(binding: ClientBinding) {
    return `${binding.framework}|${binding.entity_uri}`;
  }
}
